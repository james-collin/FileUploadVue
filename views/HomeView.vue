...
const uploadToS3 = async (file) => {
  ...
  const uniqueFileName = `${Date.now()}_${startingCellType.value}_${targetCellType.value}_${file.name}`;
  ...
  try {
    ...
    uploadedLink.value = data.Location;
  } catch (err) {
...
<template>
  <div class="flex justify-center items-center h-screen bg-gray-100">
    <div class="bg-white p-8 rounded-lg shadow-lg max-w-lg w-full">
      ...
      <div v-if="isLoading" class="w-full rounded-full text-center">
      Uploading ...
      ...
      <a :href="uploadedLink" class="text-blue-500 hover:text-blue-700 truncate block">{{ uploadedLink }}</a>
      </div>
    </div>
  </div>
</template>
